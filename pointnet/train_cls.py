...
def step(points, labels, model):
    ...
    scores, feature_transform_matrix = model(points.cuda())
    preds = torch.argmax(scores, dim=1)
    loss = F.cross_entropy(scores.cuda(), labels.cuda().to(torch.int64)) + get_orthogonal_loss(feature_transform_matrix)
    return loss, preds
