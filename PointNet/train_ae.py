...
def step(points, model):
    ...
    loss, _ = chamfer_distance(preds, points.cuda())
    return loss, preds
...
def validation_step(points, model):
    loss, preds = step(points, model)
    preds, points = preds.cuda(), points.cuda()
...
def main(args):
    ...
    model = PointNetAutoEncoder(2048, input_transform=False, feature_transform=False)
