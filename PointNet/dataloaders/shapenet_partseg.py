...
class ShapeNetPartSegDataset(torch.utils.data.Dataset):
    def __init__(self, phase: str, data_dir: str):
        ...
        self.shapenet_dir = osp.join(data_dir, "shapenet_part_seg_hdf5_data", "hdf5_data")
        ...
        self.pc_label = np.concatenate(self.pc_label, 0).astype(np.float64)
        self.class_label = np.concatenate(self.class_label, 0).astype(np.float64)
    ...
    def download_data(self):
        ...
        if not osp.exists(self.shapenet_dir):
            ...
            os.system(f"curl -o {zipfile} -k {www}")
            os.system(f"unzip {zipfile} -d   {self.shapenet_dir}")
            os.system(f"del {zipfile}")
